...
const { add_new_question, delete_questions, get_all_questions } = require('./db')
...
router.post('/questions', async(request, response) => {
  try {
    ...
    const question_created = await add_new_question(question, items, correct_item)
...
router.get('/questions', async(request, response) => {
  try {
    const questions = await get_all_questions()
...
router.delete('/questions', async(request, response) => {
  try {
    ...
    const questions_deleted = await delete_questions(Number(question_id))
